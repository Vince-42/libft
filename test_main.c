...
void	print_ft_lstmap_test_int_list(t_list *lst)
{
	while (lst)
	{
...
void	*ft_lstmap_test_times_ten(void *content)
{
	int	*ft_lstmap_test_new_int;

	ft_lstmap_test_new_int = malloc(sizeof(int));
	if (!ft_lstmap_test_new_int)
		return (NULL);
	*ft_lstmap_test_new_int = (*(int *)content) * 10;
	return (ft_lstmap_test_new_int);
}

void	ft_lstmap_test_del_int(void *content)
{
	free(content);
}
...
/* ************************************************************************** */
    ...
    printf("int list size >> %i\n", ft_lstsize(int_list));
    printf("txt list size >> %i\n", ft_lstsize(txt_list));
    printf("solo list size >> %i\n", ft_lstsize(solo_intlist));

    printf("*********** *********  ft_lstMAP ********* ***********\n");
    t_list	*ft_lstmap_test_original;
	t_list	*ft_lstmap_test_mapped;
	int		*ft_lstmap_test_a;
	int		*ft_lstmap_test_b;
	int		*ft_lstmap_test_c;

	ft_lstmap_test_original = NULL;
	ft_lstmap_test_mapped = NULL;

	ft_lstmap_test_a = malloc(sizeof(int));
	ft_lstmap_test_b = malloc(sizeof(int));
	ft_lstmap_test_c = malloc(sizeof(int));
	if (!ft_lstmap_test_a || !ft_lstmap_test_b || !ft_lstmap_test_c)
	{
		free(ft_lstmap_test_a);
		free(ft_lstmap_test_b);
		free(ft_lstmap_test_c);
		return (1);
	}
	...
	*ft_lstmap_test_c = 3;
	...
	ft_lstadd_back(&ft_lstmap_test_original, ft_lstnew(ft_lstmap_test_a));
	ft_lstadd_back(&ft_lstmap_test_original, ft_lstnew(ft_lstmap_test_b));
	ft_lstadd_back(&ft_lstmap_test_original, ft_lstnew(ft_lstmap_test_c));

	printf("\n--- ft_lstmap test ---\n");

	printf("Original list before ft_lstmap:\n");
	print_ft_lstmap_test_int_list(ft_lstmap_test_original);

	ft_lstmap_test_mapped = ft_lstmap(
			ft_lstmap_test_original,
			ft_lstmap_test_times_ten,
			ft_lstmap_test_del_int);

	printf("Mapped list, expected: 10 -> 20 -> 30 -> NULL\n");
	print_ft_lstmap_test_int_list(ft_lstmap_test_mapped);

	printf("Original list after ft_lstmap, expected: 1 -> 2 -> 3 -> NULL\n");
	print_ft_lstmap_test_int_list(ft_lstmap_test_original);

	ft_lstclear(&ft_lstmap_test_original, ft_lstmap_test_del_int);
	ft_lstclear(&ft_lstmap_test_mapped, ft_lstmap_test_del_int);

    char s1[] = "          ";
	printf("s1 is '     ' and sep is ' ' result is : '%s'",ft_strtrim(s1, " "));
}
